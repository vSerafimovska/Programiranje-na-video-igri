...
# 1.зголемување на прозорот на играта
WINDOWWIDTH = 640
WINDOWHEIGHT = 600  # 900

BOXSIZE = 20

# 1.zgolemuvanje na tablata na igrata
BOARDWIDTH = 10  # 20
BOARDHEIGHT = 40
...
# 4 Promenliva vo koja se chuva ratata na pagjanje
FALL_FREQUENCY = 0.27
...
# 2. Novi figuri koi mozhe da se kreiraat
TWO_SHAPE_TEMPLATE = [
    [".....", ".....", "..O..", "..O..", "....."],
    [".....", ".....", "..OO.", ".....", "....."],
]
...
# 3 Promenliva vo koja se chuvaat poenite potrebni da se prejde na sledno nivo
SCORE_REQ = 10
    ...
    while True:  # game loop
        ...
        if time.time() - lastFallTime > fallFreq:
            ...
            if not isValidPosition(board, fallingPiece, adjY=1):
                ...
                # 3 i 4 promenliva vo koja se kalkuliraat kompletiranite redovi
                removedLines = removeCompleteLines(board)
                score += removedLines
                # 4 ratata na pagjanje zavisi i od kompletiranite redovi
...
def calculateLevelAndFallFreq(score, change):
    global SCORE_REQ, FALL_FREQUENCY
    # Based on the score, return the level the player is on and
    # how many seconds pass until a falling piece falls one space.

    # 3 Presmetuvanje na level
    scoreCounter = score
    level = 1

    # 3 Korisnikot preshol na povisoko nivo
    while scoreCounter >= SCORE_REQ:
        scoreCounter -= SCORE_REQ
        SCORE_REQ += 3
        level += 1

    SCORE_REQ = 10

    # 4 Kalkulacija na ratata na pagjanje
    if change != -1 and change != 0:
        if change % 2 == 0:
            FALL_FREQUENCY -= level * 0.02
        else:
            FALL_FREQUENCY += level * 0.02
