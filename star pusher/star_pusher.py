...
def main():
    ...
    # 2 random meshanje na levelite
    random.shuffle(levels)

    # 2 promenlova vo koja se chuva prethodnoto nivo, so cel da ne se padne istoto
    previousLevelIndex = None
    ...
    while True:  # main game loop
        ...
        if result in ("solved", "next"):
            ...
            # 2 setiranje na prethodniot level, i random izbiranje na nov
            previousLevelIndex = currentLevelIndex
            while currentLevelIndex == previousLevelIndex:
                # Ensure the next level is different from the previous one.
            ...
            # 2 setiranje na prethdnoto nivo
    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == QUIT:
                ...
                # 1 Dodavanje na novi kontroli za dvizhenje na igracjot
                ...
                # 1 Dodavanje na novi kontroli za dvizhenje na kamerata
        ...
        for y in range(len(mapObjCopy[0])):
            if mapObjCopy[x][y] == "#":
                if (
                        ...
                        or (isWall(mapObjCopy, x + 1, y) and isWall(mapObjCopy, x, y + 1))
                        or (isWall(mapObjCopy, x, y + 1) and isWall(mapObjCopy, x - 1, y))
                        or (isWall(mapObjCopy, x - 1, y) and isWall(mapObjCopy, x, y - 1))
                ):
                    mapObjCopy[x][y] = "x"
...
def makeMove(mapObj, gameStateObj, playerMoveTo):
    ...
    gameStateObj["undoHistory"].append(
        {"player": gameStateObj["player"], "stars": gameStateObj["stars"].copy()}
    )
    ...
    if isWall(mapObj, playerx + xOffset, playery + yOffset):
        ...
        if (playerx + xOffset, playery + yOffset) in stars:
            ...
            if not isBlocked(
                    mapObj, gameStateObj, playerx + (xOffset * 2), playery + (yOffset * 2)
            ):
                # Move the star.
...
def undoMove(gameStateObj):
    """Undo the last move by restoring the previous player and stars positions."""
    if gameStateObj["undoHistory"]:
        previousState = gameStateObj["undoHistory"].pop()
        gameStateObj["player"] = previousState["player"]
        gameStateObj["stars"] = previousState["stars"]
