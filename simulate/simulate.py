...
# 2. Тајмаутот иницијално е поставен на 5
TIMEOUT = 5  # seconds before game over if no button is pushed.

# 2. Променлива во која се сместува секоја промена на шаблонот (листата)
patternChanges = 0
...
# 3. Променлива во која се чува димензијата на играта (матрицата)
DIMENSION = 2
...
def main():
    ...
    while True:  # main game loop
        ...
        if not waitingForInput:
            ...
            # 2. Се зголемува за еден листата на промени (патернот)
            patternChanges += 1

            # 2. Проверка дали бројот на промени е деллив со 10, ако е, се намалува тајмаутот.
            ...
            # 3. Се проверува дали поените на играчот се делливи со 10 за да се зголеми димензијата на матрицата
            if score % 10 == 0 and score != 0:
                # 3. Зголемување на играта
                DIMENSION += 1
            ...
            # 1. Чување на обратнатата листа (патерн)
            reversedPattern = list(reversed(pattern))

            # 1. Проверка дали играчот добро ја симулирал превртената листа
            ...
            # 1. Проверка дали играчот изгубил(погрешна симулација на превртената листа или истек на тајмаут)
...
def drawButtons():
    ...
    # 3. Додавање дополнителни копчиња ако се зголемила димензијата
    if DIMENSION > 2:
